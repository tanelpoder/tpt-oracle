...
FROM (
  ...
  SELECT 'ash_index_helper.sql' AS name, 'Index access info gathering helper' AS description, '@ash/ash_index_helper <sql_id> [<owner>.]<table_name> <from_time> <to_time>'||&nl||'@ash/ash_index_helper 8zz6y2yzdqjp0 %.% sysdate-1/24 sysdate'||&nl||'@ash/ash_index_helper % TPCDS.% sysdate-1/24 sysdate' AS usage FROM dual UNION ALL
  ...
  SELECT 'dash_index_helper.sql' AS name, 'Index access info gathering helper' AS description, '@ash/dash_index_helper <sql_id> [<owner>.]<table_name> <from_time> <to_time>'||&nl||'@ash/dash_index_helper 8zz6y2yzdqjp0 %.% sysdate-1/24 sysdate'||&nl||'@ash/dash_index_helper % TPCDS.% sysdate-1/24 sysdate' AS usage FROM dual UNION ALL
  ...
  SELECT 'tabprefs.sql' AS name, 'Display table stats gathering preferences' AS description, '@tabprefs [<owner>.]<table_name>'||&nl||'@tabprefs soe.orders'||&nl||'@tabprefs soe.%' AS usage FROM dual UNION ALL
